package com.tollge.modules.sms.dayu;
 ...
 */
@Slf4j
@Biz(value = "third://sms/qiniu", worker = true)
public class DayuVerticle extends BizVerticle {
     ...
     */
    @Path("/send")
    public void send(Message<JsonObject> msg) throws ClientException {
        JsonObject jo = msg.body();
        Preconditions.checkArgument(jo != null, "msg should not be null.");
        Preconditions.checkArgument(!StringUtil.isNullOrEmpty(jo.getString("phone")), "phone should not be null.");
        Preconditions.checkArgument(!StringUtil.isNullOrEmpty(jo.getString("template")), "template should not be null.");
        ...
        request.setPhoneNumbers(jo.getString("phone"));
        //必填:短信签名-可在短信控制台中找到
        request.setSignName("名称");
        //必填:短信模板-可在短信控制台中找到
        request.setTemplateCode(jo.getString("template"));
        //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
        //友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
        request.setTemplateParam(jo.getString(Const.PARAMS));
        ...
        if (sendSmsResponse.getCode() != null && sendSmsResponse.getCode().equals("OK")) {
            ...
            msg.reply(sendSmsResponse.getCode());
        }
    }
